...
M.World=World;
M.makeSky=function(scene){const sky=new T.Mesh(new T.SphereGeometry(1500,32,16),new T.ShaderMaterial({side:T.BackSide,depthWrite:false,uniforms:{top:{value:new T.Color('#087cea')},horizon:{value:new T.Color('#a5e8ff')},sunDir:{value:V(-.75,.23,-.6).normalize()}},vertexShader:'varying vec3 v;void main(){v=position;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}',fragmentShader:'varying vec3 v;uniform vec3 top;uniform vec3 horizon;uniform vec3 sunDir;void main(){vec3 d=normalize(v);float h=max(d.y,0.);vec3 col=mix(horizon,top,pow(h,.48));float su=max(dot(d,sunDir),0.);col+=vec3(1.,.55,.25)*pow(su,32.)*.25;col+=vec3(1.,.88,.64)*smoothstep(.99997,.99999,su)*2.;gl_FragColor=vec4(col,1.);}'}));scene.add(sky);return sky;};
})(Marlon);
